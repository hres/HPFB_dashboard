...
mhpd_cr<-read.xlsx('_For testing/MHPD/Operational Dashboard - post-market data.xlsx',3,rows=c(2:47))

main_list<-mhpd_cr$X1[is.na(mhpd_cr$X2)]
sub_list<-mhpd_cr$X1[!is.na(mhpd_cr$X2)]
sub_list<-sub_list[!is.na(sub_list)]
mhpd_cal<-as.data.frame(as.matrix(mhpd_cr[grepl('On time',mhpd_cr$X2),c(3:15)])/as.matrix(mhpd_cr[grepl('Total',mhpd_cr$X2),c(3:15)]))
mhpd_cal$category<-sub_list

index_main_list<-c(which(is.na(mhpd_cr$X2)),nrow(mhpd_cr)+1)
main_list_count<-(diff(index_main_list,lag=1)-1)/2
main_list<-unlist(mapply(rep,main_list,main_list_count,USE.NAMES =F,simplify=TRUE))
mhpd_cal$cat2<-main_list

mhpd_cal<-mhpd_cal%>%
  gather(month,percent,-category,-cat2)%>%
  filter(!month %in% c('YTD','Current.Month'))
